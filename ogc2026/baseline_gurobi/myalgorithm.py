...
def spatiotemporal_shaw_removal(state, prob_info, bays, num_remove):
    # 무작위로 기준 블록을 하나 선택한 후, 시공간 밀접도를 계산하여 확률적으로 블록들을 파괴
    if not state: return set()
    base_b_id = random.choice(list(state.keys()))
    base_val = state[base_b_id]
    
    relatedness = []
    for b_id, val in state.items():
        if b_id == base_b_id:
            continue
        
        bay_i, x_i, y_i, o_i, entry_i, exit_i = base_val
        bay_j, x_j, y_j, o_j, entry_j, exit_j = val
        
        overlap = max(0, min(exit_i, exit_j) - max(entry_i, entry_j))
        ...
        score = - (overlap * 10) + dist * 0.1
        relatedness.append((b_id, score))
        
    relatedness.sort(key=lambda x: x[1])
    
    removed = {base_b_id}
    candidates = [x[0] for x in relatedness]
    while len(removed) < min(num_remove, len(state)) and candidates:
        idx = int((random.random() ** 3) * len(candidates))
        removed.add(candidates.pop(idx))
...
def is_physical_conflict(c1, c2, prob_info, bays):
    # 두 후보 배치 상태 간의 물리적 충돌 여부를 검사
...
def is_physical_conflict_with_fixed(c1, p_blk, p_entry, p_exit, bay, prob_info):
    blk1 = Block(block_id=c1['id'], block_data=prob_info['blocks'][c1['id']], x=c1['x'], y=c1['y'], orient_idx=c1['o_idx'])
    if check_collisions(bay, [blk1, p_blk]): return True
    if check_entry(bay, [p_blk], blk1, fast=True): return True
    if check_exit(bay, [p_blk], blk1, fast=True): return True
    if check_entry(bay, [blk1], p_blk, fast=True): return True
    if check_exit(bay, [blk1], p_blk, fast=True): return True
    return False
...
def generate_candidates(b_id, b_info, bays, bay_placed, bay_schedule, raster_cache, num=20):
    ...
    for bay_idx in bay_order:
        bay = bays[bay_idx]
        for o_idx in range(len(b_info['shape'])):
            for t_offset in [-2, -1, 0, 1, 2, 3]:
                exit_t = d_time + t_offset
                entry = exit_t - p_time
                if entry < r_time:
                    continue
                ...
                for gx, gy, score in spots:
                    ...
                    if not has_col:
                        ...
                        if not obs_failed and not check_entry(bay, p_in, new_blk, fast=True) and not check_exit(bay, p_out, new_blk, fast=True):
                            ...
                            if len(cands) >= num * 2: return cands
...
def repair_gurobi(U, fixed_state, prob_info, bays, raster_cache, time_limit):
    # 파괴된 블록들을 Gurobi 정수 계획법(Integer Programming)을 통해 최적의 시간 및 공간 좌표로 재배치
    bay_placed = {bay.id: [] for bay in bays}
    bay_schedule = {bay.id: [] for bay in bays}
    ...
    for b_id in U:
        b_info = prob_info['blocks'][b_id]
        cands = generate_candidates(b_id, b_info, bays, bay_placed, bay_schedule, raster_cache, num=10)
        if not cands: return None
        unique_cands = []
        seen = set()
        for c in cands:
            key = (c['bay'], c['x'], c['y'], c['o_idx'])
            if key not in seen:
                seen.add(key)
                unique_cands.append(c)
        candidates_by_block[b_id] = unique_cands

    try:
        env = gp.Env(empty=True)
        env.setParam('OutputFlag', 0)
        env.start()
        model = gp.Model("repair", env=env)
        ...
        for b_id, cands in candidates_by_block.items():
            r_time = prob_info['blocks'][b_id]['release_time']
            p_time = prob_info['blocks'][b_id]['processing_time']
            E[b_id] = model.addVar(vtype=GRB.INTEGER, lb=r_time, name=f"E_{b_id}")
            Ex[b_id] = model.addVar(vtype=GRB.INTEGER, lb=r_time + p_time, name=f"Ex_{b_id}")
            model.addConstr(Ex[b_id] == E[b_id] + p_time)
            
            for c_idx in range(len(cands)):
                X[(b_id, c_idx)] = model.addVar(vtype=GRB.BINARY, name=f"X_{b_id}_{c_idx}")
            model.addConstr(gp.quicksum(X[(b_id, c_idx)] for c_idx in range(len(cands))) == 1)
            ...
            for j in range(i+1, len(U_list)):
                ...
                for c1_idx, c1 in enumerate(candidates_by_block[b1]):
                    for c2_idx, c2 in enumerate(candidates_by_block[b2]):
                        if is_physical_conflict(c1, c2, prob_info, bays):
                            conflicting_cands.append((c1_idx, c2_idx))
                            
                if conflicting_cands:
                    seq12 = model.addVar(vtype=GRB.BINARY, name=f"seq_{b1}_{b2}")
                    seq21 = model.addVar(vtype=GRB.BINARY, name=f"seq_{b2}_{b1}")
                    
                    model.addGenConstrIndicator(seq12, True, Ex[b1] <= E[b2])
                    model.addGenConstrIndicator(seq21, True, Ex[b2] <= E[b1])
                    
                    for c1_idx, c2_idx in conflicting_cands:
                        model.addConstr(seq12 + seq21 >= X[(b1, c1_idx)] + X[(b2, c2_idx)] - 1)

        for b1 in U_list:
            for c1_idx, c1 in enumerate(candidates_by_block[b1]):
                bay_id = c1['bay']
                for p_idx, (p_blk, (p_entry, p_exit)) in enumerate(zip(bay_placed[bay_id], bay_schedule[bay_id])):
                    if is_physical_conflict_with_fixed(c1, p_blk, p_entry, p_exit, bays[bay_id], prob_info):
                        seq1f = model.addVar(vtype=GRB.BINARY)
                        seqf1 = model.addVar(vtype=GRB.BINARY)
                        model.addGenConstrIndicator(seq1f, True, Ex[b1] <= p_entry)
                        model.addGenConstrIndicator(seqf1, True, p_exit <= E[b1])
                        model.addConstr(seq1f + seqf1 >= X[(b1, c1_idx)])
        ...
        Tard = {}
        for b_id in U_list:
            due_date = prob_info['blocks'][b_id]['due_date']
            Tard[b_id] = model.addVar(vtype=GRB.CONTINUOUS, lb=0.0)
            model.addConstr(Tard[b_id] >= Ex[b_id] - due_date)
            obj_expr += w1 * Tard[b_id]
        ...
        if model.status in [GRB.OPTIMAL, GRB.TIME_LIMIT, GRB.INTERRUPTED] and model.SolCount > 0:
            ...
            for b_id, cands in candidates_by_block.items():
                sel_c = -1
                for c_idx in range(len(cands)):
                    if X[(b_id, c_idx)].X > 0.5:
                        sel_c = c_idx
                        break
                if sel_c >= 0:
                    cand = cands[sel_c]
                    e_val = int(round(E[b_id].X))
                    ex_val = int(round(Ex[b_id].X))
                    new_state[b_id] = (cand['bay'], cand['x'], cand['y'], cand['o_idx'], e_val, ex_val)
            return new_state
        return None
...
def alns_worker(prob_info, bays, initial_state, initial_obj, timelimit, start_time, seed, alns_duration):
    ...
    while True:
        ...
        operator = random.choices([0, 1, 2, 3], weights=alns_weights)[0]
        ...
        elif operator == 2: removed = destroy_tardiness(current_state, prob_info, curr_remove)
        else: removed = spatiotemporal_shaw_removal(current_state, prob_info, bays, curr_remove)
            
        fixed_state = {k: v for k, v in current_state.items() if k not in removed}
        adaptive_limit = max(3.0, min(12.0, len(removed) * 0.4))
        new_state = repair_gurobi(removed, fixed_state, prob_info, bays, raster_cache, time_limit=adaptive_limit)
